...
13. HTTP is a stateless protocol, meaning that each request/response cycle is independent of any previous or subsequent cycles.
14. HTTP cookies are small pieces of data that a server can store on a client's device to remember user preferences, login credentials, and other information.
15. The WebSocket protocol is an alternative to HTTP that allows for bidirectional communication between a client and a server, making it useful for real-time applications such as chat and gaming.
16. The fetch() method is a modern alternative to XMLHttpRequest (XHR) for making HTTP requests in JavaScript. It is more streamlined and easier to use than XHR, and supports promises for asynchronous programming.
17. Node.js is a popular platform for building server-side applications using JavaScript, and includes a built-in HTTP module for handling HTTP requests and responses.
18. JSON (JavaScript Object Notation) is a lightweight data interchange format that is widely used for transferring data between a client and a server over HTTP.
19. CORS (Cross-Origin Resource Sharing) is a security feature that allows a server to indicate which origins (domains, protocols, and ports) are allowed to access its resources, helping to prevent malicious attacks.
20. Regular expressions (regex) are powerful tools for pattern matching and text manipulation in JavaScript and other programming languages, and are often used for validating user input and parsing text data.
21. REST (Representational State Transfer) is a architectural style for building web APIs that uses HTTP verbs (GET, POST, PUT, DELETE) and URLs to represent resources and actions.
22. MVC (Model-View-Controller) is a software design pattern that separates an application into three interconnected components: the model (data), the view (user interface), and the controller (logic). It is commonly used in web development frameworks such as Ruby on Rails and Django.
